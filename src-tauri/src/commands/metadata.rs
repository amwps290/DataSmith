use crate::database::{ColumnInfo, DatabaseInfo, IndexInfo, QueryResult, SchemaInfo, FunctionInfo, ExtensionInfo};
use crate::AppState;
use tauri::State;
...
#[tauri::command]
#[instrument(skip(state))]
pub async fn get_tables(connection_id: String, database: Option<String>, state: State<'_, AppState>) -> Result<Vec<crate::database::TableInfo>, String> {
    state.connection_manager.get_tables(&connection_id, database.as_deref()).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn get_schema_tables(connection_id: String, database: String, schema: String, state: State<'_, AppState>) -> Result<Vec<crate::database::TableInfo>, String> {
    state.connection_manager.get_tables(&connection_id, Some(&database)).await.map(|tables| {
        tables.into_iter().filter(|t| t.schema.as_deref() == Some(&schema)).collect()
    }).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn get_views(connection_id: String, database: Option<String>, state: State<'_, AppState>) -> Result<Vec<crate::database::TableInfo>, String> {
    state.connection_manager.get_views(&connection_id, database.as_deref()).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn get_schema_views(connection_id: String, database: String, schema: String, state: State<'_, AppState>) -> Result<Vec<crate::database::TableInfo>, String> {
    state.connection_manager.get_views(&connection_id, Some(&database)).await.map(|views| {
        views.into_iter().filter(|v| v.schema.as_deref() == Some(&schema)).collect()
    }).map_err(|e| e.to_string())
}
...
pub async fn get_schema_indexes(connection_id: String, database: String, schema: String, state: State<'_, AppState>) -> Result<Vec<IndexInfo>, String> {
    // 简化实现：目前驱动层主要是按表查索引，此处返回空或实现全 schema 索引查询
    Ok(vec![])
}

// 占位指令，后续完善
#[tauri::command] pub async fn view_table_data(_c: String, _t: String, _d: Option<String>, _s: Option<String>) -> Result<QueryResult, String> { Err("Not implemented".into()) }
